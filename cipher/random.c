...
static int (*getfnc_gather_random(void))(void (*)(const void *, size_t, int),
                                         int, size_t, int) {
...
static int (*getfnc_fast_random_poll(void))(void (*)(const void *, size_t, int),
                                            int) {
#ifdef USE_RNDW32
...
static void burn_stack(int bytes) {
    ...
    bytes -= sizeof buf;
    if (bytes > 0) burn_stack(bytes);
}

void random_dump_stats() {
    fprintf(stderr,
...
void secure_randoxmalloc() { secure_alloc = 1; }
...
void random_disable_locking() { no_seed_file_locking = 1; }
...
byte *get_random_bits(size_t nbits, int level, int secure) {
    ...
    /* trollwot addition: instead of randomly generating bits, just return
 ...
 * Mix the pool
 */
static void mix_pool(byte *pool) {
...
static int lock_seed_file(int fd, const char *fname, int for_write) {
    ...
    if (no_seed_file_locking) return 0;
...
void update_random_seed_file() {
    ...
    if (!seed_file_name || !is_initialized || !pool_filled) return;
    ...
    for (i = 0, dp = (ulong *)keypool, sp = (ulong *)rndpool; i < POOLWORDS;
         i++, dp++, sp++) {
        *dp = *sp + ADD_VALUE;
    }
    mix_pool(rndpool);
     ...
     * readers.  Loop until success.  Complain after a few failures.  */
    {
        ...
        while ((fd = open(seed_file_name, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR,
                          "shr=get")) == -1) {
...
static void read_pool(byte *buffer, size_t length, int level) {
    ...
    if (!pool_filled) {
        if (read_seed_file()) pool_filled = 1;
    }
    ...
    if (level == 2 && pool_balance < length) {
        ...
        if (pool_balance < 0) pool_balance = 0;
        needed = length - pool_balance;
        if (needed > POOLSIZE) BUG();
        read_random_source(3, needed, 2);
        pool_balance += needed;
    }

    /* make sure the pool is filled */
    while (!pool_filled) random_poll();
    ...
    if (!level) { /* no need for cryptographic strong random */
        ...
        while (length--) {
            *buffer++ = keypool[pool_readpos++];
            if (pool_readpos >= POOLSIZE) pool_readpos = 0;
            pool_balance--;
        }
        if (pool_balance < 0) pool_balance = 0;
...
void fast_random_poll() {
    ...
    {
        struct timespec tv;
        if (clock_gettime(CLOCK_REALTIME, &tv) == -1) BUG();
        add_randomness(&tv.tv_sec, sizeof(tv.tv_sec), 1);
...
static void read_random_source(int requester, size_t length, int level) {
    ...
    if (!fnc) {
        ...
        if (!requester && !length && !level) return; /* init only */
    }
...
static int gather_faked(void (*add)(const void *, size_t, int), int requester,
    ...
    if (!initialized) {
        ...
        tty_printf(
            _("The random number generator is only a kludge to let\n"
